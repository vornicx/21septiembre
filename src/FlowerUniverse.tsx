...
export default function FlowerUniverse({ onClose }: { onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const flowers=Array.from({length:1600},(_,i)=>({kind:i%sprites.length,x:(random()<.5?-1:1)*(.24+random()*8),z:random()*26,size:.7+random()*.55,phase:random()*6.28}));
    ...
    function draw(){
      ...
      for(const f of depth){const scale=1/f.d;const x=w/2+f.x*w*.8*scale;const y=horizon+h*1.15*scale;const size=Math.min(w,h)*.32*scale*f.size;if(x < -size||x>w+size)continue;ctx.globalAlpha=Math.min(1,.42+scale*4)*Math.min(1,(26.35-f.d)*2);const sway=paused?0:Math.sin(time*.7+f.phase)*size*.035;ctx.drawImage(sprites[f.kind],x-size/2+sway,y-size*1.5,size,size*1.5);}
      ctx.globalAlpha=1;
  ...
  return <dialog className="universe-dialog travelling-garden" ref={dialog} onCancel={onClose} aria-labelledby="universe-title">
    ...
    <div className="universe-dedication"><p>UN PASEO ENTRE ROSAS</p><h2 id="universe-title">Haridian,<br/><em>te quiero.</em></h2><span>Qué ganas de pasear contigo.</span></div>
