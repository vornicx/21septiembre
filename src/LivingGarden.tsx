...
export default function LivingGarden({paused}:{paused:boolean}) {
 ...
 useEffect(()=>{
  ...
  function render(now:number){const dt=Math.min((now-last)/1000,.05);last=now;if(ready&&!document.hidden){if(!pausedRef.current){t+=dt;x+=(tx-x)*Math.min(1,dt*3);y+=(ty-y)*Math.min(1,dt*3);}gl.uniform1f(time,t);gl.uniform2f(pointer,x,y);gl.drawArrays(gl.TRIANGLES,0,6);}raf=requestAnimationFrame(render);}
 ...
 return <><img className="garden-fallback" src="/art/rose-field.webp" alt="Rosales amarillos al atardecer"/>{!failed&&<canvas className="living-garden" ref={ref} tabIndex={0} aria-label="Campo interactivo de rosas. Mueve el ratón, arrastra con el dedo o usa las flechas para mirar alrededor."/>}<div className="floating-petals" aria-hidden="true">{Array.from({length:16},(_,i)=><span key={i} style={{left:`${i*6.7}%`,animationDelay:`${-i*2.1}s`,animationDuration:`${15+i%5*3}s`}}/>)}</>;
}
